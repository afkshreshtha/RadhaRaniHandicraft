  ...
  while ((match = LINK_RE.exec(text)) !== null) {
    ...
    parts.push(
      <a
        ...
        title={isPlaceholder ? "Link not yet set — update in Sanity Studio" : undefined}
      >
        {linkText}
      </a>
    )
...
function RenderBlockWithPlaceholders({
  ...
  return (
    ...
    <Tag className={className}>
      {parseLinkPlaceholders(fullText)}
    </Tag>
  )
}
// ─────────────────────────────────────────────────────────────────────────────
  ...
  while (i < blocks.length) {
          ...
          >
            ...
            <div className="px-7 divide-y divide-stone-50">
              {faqPairs.map((pair, idx) => (
                <FAQItem key={idx} question={pair.question} answer={pair.answer} index={idx} />
              ))}
            </div>
          </section>
        )
    ...
    if (style === "h4") {
      ...
      if (numMatch) {
        ...
        nodes.push(
          <h4 key={block._key ?? i} className="font-serif text-lg font-bold text-stone-900 mt-8 mb-3">
            {blockHasLinkPlaceholders(block) ? (
              parseLinkPlaceholders(text)
            ) : (
              <RenderChildren children={block.children} markDefs={block.markDefs} />
...
function TableOfContents({ blocks }: { blocks: any[] }) {
  const headings = blocks
    ...
    .map((b) => ({
      text: getBlockText(b),
      level: b.style,
      id: getBlockText(b).toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""),
    }))
  ...
  return (
    <nav aria-label="Table of contents" className="hidden xl:block sticky top-28 w-56 flex-shrink-0 self-start">
      <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-stone-400 mb-4">In this article</p>
          ...
          <li key={h.id}>
            <a
              ...
              className={`block text-[13px] leading-snug py-1 transition-colors hover:text-amber-700 ${
                h.level === "h3" ? "pl-3 text-stone-400" : "text-stone-500 font-medium"
              }`}
              onClick={(e) => {
                e.preventDefault()
...
const BookmarkIcon = ({ filled }: { filled: boolean }) => (
  <svg width="15" height="15" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
...
function ReadingProgress() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-[3px] bg-stone-100" role="progressbar" aria-label="Reading progress" aria-valuenow={Math.round(progress)}>
      <div
    ...
    >
      <div className="aspect-[16/9] overflow-hidden bg-stone-100">
        {post.featuredImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-amber-50" aria-hidden>
      ...
      <div className="p-5 flex flex-col gap-2 flex-1">
        ...
        {post.excerpt && (
          <p className="text-[13px] text-stone-500 line-clamp-2 leading-relaxed">{post.excerpt}</p>
        )}
...
export default function BlogDetailClient({ post }: { post: Post }) {
  ...
  const hasToc = post.content && post.content.filter(
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f9f7f4] font-ui">
        ...
        >
          <div className="max-w-6xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {post.readTime && (
                <span className="hidden sm:block text-[11px] text-stone-400">{post.readTime} min read</span>
              )}
              <button
                onClick={copyLink}
        ...
        {post.featuredImage && (
          <div className="relative w-full overflow-hidden bg-stone-900" style={{ paddingTop: "56px" }}>
            <img
              ...
              style={{ maxHeight: "55vh", minHeight: "300px", objectFit: "cover" }}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-stone-900/70 via-stone-900/10 to-transparent" aria-hidden />
          </div>
        )}
          ...
          >
            <nav aria-label="Breadcrumb" className="mb-5">
              <ol className="flex items-center gap-2 flex-wrap">
                <li>
                  <Link href="/" className="text-[11px] text-stone-400 hover:text-amber-700 transition-colors">Home</Link>
                </li>
                <li aria-hidden className="text-stone-300 text-[11px]">/</li>
                <li>
                  <Link href="/blog" className="text-[11px] text-stone-400 hover:text-amber-700 transition-colors">Blog</Link>
                </li>
                {post.category && (
                  <>
                    <li aria-hidden className="text-stone-300 text-[11px]">/</li>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 pt-10 pb-24">
          <div className="flex gap-10 lg:gap-14 items-start justify-center">
            ...
            <article ref={articleRef} className="flex-1 min-w-0 max-w-[700px]" aria-label={post.title}>
              {hasContent ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${
                      copied ? "bg-green-500 border-green-500 text-white" : "border-stone-200 bg-white text-stone-400 hover:border-stone-400"
                    }`}
                  >
                    <LinkIcon />
                  </button>
            ...
            {hasToc && post.content && <TableOfContents blocks={post.content} />}
          </div>
        </div>
            ...
            <div className="max-w-6xl mx-auto px-4 md:px-8 py-16">
              <div className="flex items-end justify-between mb-9 gap-4">
                <div>
                  ...
                  <h2 className="font-serif text-2xl md:text-3xl font-bold text-stone-900">Related Articles</h2>
                </div>
